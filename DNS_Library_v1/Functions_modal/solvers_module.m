function [ICvkron,ICvDvkron,ICgkron,ICggkron,S_mf,S_mp,Sol_m,kkm,llm]=solvers_module(g,h)

%%% Mean flow and v,g solver matrices with timestep of fraction h

global N NX MZ D2 D2F k l D4 kLyap NLyap msolv Nmsolv
  ...
  for ii=1:NX/3
    ...
    ICvIDELv(:,:,ii,kk)=ICv*IDEL;
          ...
          ICvkron=spalloc(N*NL*Nmsolv,N*NL*Nmsolv,N*N*NL*Nmsolv);
          ICvDvkron=ICvkron;
          ICgkron=ICvkron;
          ICggkron=ICvkron;
          kkm=spalloc(N*NL*Nmsolv,N*NL*Nmsolv,N*NL*Nmsolv);
          llm=kkm;
...
for ii=kLyap+1
    ...
    ei=zeros(NL,1);ei(sum(NLyap(1:Lkiter-1))/2+1:sum(NLyap(1:Lkiter))/2)=1;
    
    IM=spdiags(ei,0,NL,NL);
