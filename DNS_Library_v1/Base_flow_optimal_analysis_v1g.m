...
[uLn,vLn,wLn,gLn,Lkn,gmold,rtold]=initialize_dist(field_path,start_opt,noise_file,cont_old);
...
%% Iteration loop
  ...
  tic;  
    ...
    [uLn,vLn,wLn,gLn,cgr(itc,:)] = opti_v2_module(uLn,vLn,wLn,gLn,umean1,vmean1,wmean1,gmean1,NT,h,Lkn);
   ...
   % stop_flags    
        ...
        plot(Lin(1:2:end)+1-Lin(1),cgr(end,Lin(1:2:end)),stln{Lkk})
        hold on
        title(['iter = ',num2str(cvstep)])
        end
        
        figure(140);clf
        for Lkk=1:length(kLyap)
        Lin = find( Lkn == kLyap(Lkk));
        plot(1:cvstep,cgr(1:cvstep,Lin),stln{Lkk})
        hold on
        end
...
%             [uL1,vL1,wL1,gL1]=pick_vec(uL,vL,wL,gL,lcount2,kcount2);
