...
ghat=fft2_cube(g0);g1v=reshape(permute(ghat(2:end-1,ksolv,mrange),[1 3 2]),[N*(Nksolv)*(Nmsolv),1]);
vvhat=fft2_cube(v0);v1v=reshape(permute(vvhat(2:end-1,ksolv,mrange),[1 3 2]),[N*(Nksolv)*(Nmsolv),1]);
ag1hat=fft2_cube(advg1)*(-dt)*h;ag1v=reshape(permute(ag1hat(2:end-1,ksolv,mrange),[1 3 2]),[N*(Nksolv)*(Nmsolv),1]);
av1hat=fft2_cube(advv1)*(-dt)*h;av1v=reshape(permute(av1hat(2:end-1,ksolv,mrange),[1 3 2]),[N*(Nksolv)*(Nmsolv),1]);
...
%Rebuild
gi(2:end-1,ksolv,mrange)=ipermute(reshape(g1v,[N Nmsolv Nksolv]),[1 3 2]);
vi(2:end-1,ksolv,mrange)=ipermute(reshape(v1v,[N Nmsolv Nksolv]),[1 3 2]);
ui(2:end-1,ksolv,mrange)=ipermute(reshape(u1v,[N Nmsolv Nksolv]),[1 3 2]);
wi(2:end-1,ksolv,mrange)=ipermute(reshape(w1v,[N Nmsolv Nksolv]),[1 3 2]);
