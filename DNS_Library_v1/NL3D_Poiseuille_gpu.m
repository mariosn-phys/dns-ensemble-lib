...
% Base DNS code used to simulate plane Couette flow in Nikolaidis & Ioannou (2022)
% A pseudo-spectal Navier-Stokes solver for plane parallel flows
...
global MZ k l dt xE yE zE A B L dx dz kkm llm DYkron ksolv msolv Nksolv Nmsolv
...
for it=it0:NT
   ...
   %end
    ...
    gmf=(1/2/Re*(O_top(it-1)-O_bot(it-1))*ones(N+2,1))*pm+gm*pp;
    advU1=DYF*mean(mean(up.*vp,3),2)+gmf;
    ...
    gmf=(1/2/Re*(DYF(end,:)*UP1-DYF(1,:)*UP1)*ones(N+2,1))*pm+gm*pp;
    advU2=DYF*mean(mean(up.*vp,3),2)+gmf;
    ...
    gmf=(1/2/Re*(DYF(end,:)*UP2-DYF(1,:)*UP2)*ones(N+2,1))*pm+gm*pp;
    advU3=DYF*mean(mean(up.*vp,3),2)+gmf;
